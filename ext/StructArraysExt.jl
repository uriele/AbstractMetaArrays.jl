...
@forward AbstractMetaArray._data Tables.columns,
Tables.schema,
Tables.materializer,
Tables.isrowtable,
Tables.columnaccess
...
function AbstractMetaArrays._metacomponent(
        x::S,
        key::Symbol
) where {S <: StructArray{T, N}} where {T, N}
    getproperty(x, key)
end
...
function Base.showarg(
        ...
        ma::MA,
        toplevel
) where {MA <: AbstractMetaArray{T, N, <:StructArray}} where {T, N}
